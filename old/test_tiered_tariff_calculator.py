...
def test_all_finite_blocks_raises():
    with pytest.raises(ValueError, match="At least one tier must have block_kwh=None"):
        calculate_tiered_bill(1, [(100, 0.2), (200, 0.3)], 0)
...
# -------------------------
# Branch/Statement coverage helpers
# -------------------------

@pytest.mark.parametrize("consumption, tiers, fixed", [
    (150, [(100, 0.2), (None, 0.3)], 0.0),  # cover both finite and unlimited blocks
    (1,   [(None, 0.5)],  0.0),             # single unlimited block
])
